...
import { OsintError, OsintLoading } from "@/components/OsintFeedback";
...
export default function OsintCatalogPanel() {
  ...
  return <Card className="border-red-300/15 bg-gradient-to-br from-red-500/[0.08] to-white/[0.025]">
    ...
    <CardContent>{modules.isLoading ? <OsintLoading label="Memuat katalog OSINT…" detail="Menyiapkan 19 modul dan kebijakan keamanan." /> : modules.isError ? <OsintError error={modules.error} onRetry={() => modules.refetch()} title="Katalog OSINT tidak dapat dimuat" /> : <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{visible.map((item) => <article key={item.id} className="rounded-xl border border-white/10 bg-black/20 p-4 transition hover:border-red-300/25"><div className="flex items-start justify-between gap-3"><h3 className="text-sm font-medium text-zinc-100">{item.name}</h3><Badge variant="outline" className="border-white/10 text-[10px] text-zinc-500">OSINT</Badge></div><p className="mt-2 text-xs leading-5 text-zinc-500">{item.description}</p><div className="mt-3 flex items-center gap-2 text-[11px] text-emerald-300"><ShieldCheck className="h-3.5 w-3.5" /> Scope + audit wajib</div><p className="mt-2 text-[11px] leading-4 text-zinc-600">Keluaran: {item.output ?? "evidence tersanitasi"}</p></article>)}</div>}{!modules.isLoading && !modules.isError && !visible.length && <p className="py-8 text-center text-sm text-zinc-600">Modul tidak ditemukan pada filter ini.</p>}{!modules.isLoading && !modules.isError && <p className="mt-4 text-xs text-zinc-600">Menampilkan {visible.length} dari {osintModules.length} modul. Tidak ada credential, password, token, atau data privat yang dikumpulkan.</p>}</CardContent>
  </Card>;
}
